...
/*!***************************************************************************************!*\
  !*** ./src/scripts/custom/use-case-to-learn-promise/use-case-03-batching-requests.js ***!
  \***************************************************************************************/
__webpack_require__.r(__webpack_exports__);
...
var fetchAddressesInBatches = function fetchAddressesInBatches(users) {
  var batchSize = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 5;
  var batches = [];
  for (var i = 0; i < users.length; i += batchSize) {
    var batch = users.slice(i, i + batchSize).map(function (user) {
      return fetchUserAddress(user.id);
    });
    batches.push(Promise.all(batch));
  }
  return batches.reduce(function (promiseChain, currentBatch) {
    return promiseChain.then(function (results) {
      return currentBatch.then(function (currentResults) {
        return [].concat(_toConsumableArray(results), _toConsumableArray(currentResults));
      });
    });
  }, Promise.resolve([]));
};
var fetchAndCombineData = function fetchAndCombineData() {
  fetchUsers().then(function (users) {
    ...
    return fetchAddressesInBatches(users).then(function (addresses) {
      return {
        users: users,
        addresses: addresses,
...
/******/ })()
;
//# sourceMappingURL=use-case-03-batching-requests.js.map