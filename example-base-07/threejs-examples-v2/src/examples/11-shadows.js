...
export function createExample(container) {
  ...
  const floor=new THREE.Mesh(new THREE.PlaneGeometry(20,20),new THREE.MeshStandardMaterial({color:0x64748b}));
  floor.rotation.x=-Math.PI/2;floor.receiveShadow=true;scene.add(floor);

  const box=new THREE.Mesh(new THREE.BoxGeometry(2,2,2),new THREE.MeshStandardMaterial({color:0xf97316}));
  box.position.y=1;box.castShadow=true;scene.add(box);

  const light=new THREE.DirectionalLight(0xffffff,4);
  light.position.set(4,7,4);light.castShadow=true;scene.add(light);
  scene.add(new THREE.AmbientLight(0xffffff,.25));

  let running=true;
  function animate(){if(!running)return;requestAnimationFrame(animate);box.rotation.y+=.01;renderer.render(scene,camera);}
